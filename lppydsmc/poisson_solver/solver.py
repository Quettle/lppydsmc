...
def solver(mesh, boundary_conditions, charge_density):
    ...
    V = FunctionSpace(mesh = mesh, 'Lagrange', degree = 1)
    
    # Defining boundary conditions 
    bc_list = []
    for key, value in boundary_conditions.item():
        ...
        if(boundary_conditions['type'] == 'Dirichlet'):
            ...
            u_d = Expression(boundary_conditions['solution'], degree=boundary_conditions['deg'], **boundary_conditions['solution_kwargs']) # degree should be at least 1 - better too for more accuracy
            bc_list.append(DirichletBC(V, u_d, boundary_conditions['boundary']), **boundary_conditions['boundary_kwargs']) # boundary defines a subdomain of the main domain - and it is a string

    # Defining source term
    charge_density = Expression('{}/VACUUM_PERMITTIVITY'.format(charge_density['rho']), degree=charge_density['deg'], VACUUM_PERMITTIVITY = VACUUM_PERMITTIVITY) # Note : optimization can be made in case the source_term is constant
