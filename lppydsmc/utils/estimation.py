...
# Generally based on sampling techniques (Monte Carlo methods etc.)

# --------------- Check if points is inside a polygone ---------------  #
...
def estimate_surface(samples, points):
    ...
    X_samples = min_x + np.random.random(size = (N))*(max_x-min_x)
    Y_samples = min_y + np.random.random(size = (N))*(max_y-min_y)  
    sample_points = np.stack((X_samples, Y_samples), axis = 1)

    results = np.zeros((samples))
    for k, point in enumerate(sample_points) : 
        b = is_inside_polygon(list(points),tuple(point))
        if(b):
            results[k] = 1

    results = results * rectangle_area
